...
if DEMO_MODE:
    ...
    demo_fake_selected_label = st.sidebar.selectbox(
    ...
    if use_sma:
        st.plotly_chart(
            plot_price_with_sma(
                view, selected_coin, [f"sma_{int(sma_fast)}", f"sma_{int(sma_slow)}"]
            ),
...
if run:
    ...
    if posts_choice in ("All", "News"):
        ...
        news_df = load_cached_csv(
            news_settings, parse_dates=["timestamp"], freshness_minutes=30
        )
        if news_df is None:
            with st.spinner("Fetching news..."):
                ...
                mask = (news_df["timestamp"] >= start_date) & (
    ...
    if posts_choice in ("All", "Reddit"):
        ...
        reddit_df = load_cached_csv(
            reddit_settings, parse_dates=["timestamp"], freshness_minutes=30
        )
        if reddit_df is None:
            with st.spinner("Fetching Reddit posts..."):
                reddit_df = fetch_reddit_posts(
                    query=reddit_settings["query"],
                    limit=num_posts,
                    start_date=start_date,
                    end_date=end_date,
                    subreddits=subreddits,  # type: ignore ; treated in declaration
                )
                reddit_df["source"] = "reddit"
                reddit_df["timestamp"] = pd.to_datetime(
                    reddit_df["timestamp"], utc=True
                )

                mask = (reddit_df["timestamp"] >= start_date) & (
                    reddit_df["timestamp"] <= end_date
                )
    ...
    if posts_choice in ("All", "Twitter/X"):
        ...
        tweets_df = load_cached_csv(
            twitter_settings, parse_dates=["timestamp"], freshness_minutes=30
        )
    ...
    with st.spinner("Combining sentiment..."):
        ...
        if not dfs:
            logging.error(
                f"No sentiment data. use_news={use_news}, use_reddit={use_reddit}, use_twitter={use_twitter}"
            )
            st.error(
                "No sentiment data could be loaded. Check API limits or local files."
            )
            st.stop()

        combined_df = pd.concat(dfs, ignore_index=True)
        combined_df["timestamp"] = pd.to_datetime(
            combined_df["timestamp"], errors="coerce", utc=True
        ).dt.tz_localize(None)
        combined_df["source"] = (
            combined_df["source"].astype(str).str.strip().str.lower()
        )
        combined_df = combined_df.dropna(subset=["timestamp"]).sort_values("timestamp")

        logging.info(
            "Combined counts by source: %s",
            combined_df["source"].value_counts(dropna=False).to_dict(),
        )
        combined_sentiment_path = get_data_path(selected_coin, "combined_sentiment")
    ...
    price_df = load_cached_csv(
        price_settings, parse_dates=["timestamp"], freshness_minutes=30
    )
    if price_df is None:
    ...
    merged_df = load_cached_csv(
        merged_settings, parse_dates=["timestamp"], freshness_minutes=30
    )
    if merged_df is None:
        with st.spinner("Merging sentiment and price data..."):
            merge_sentiment_and_price(
                get_data_path(selected_coin, "combined_sentiment"),
            ...
            merged_df = add_indicators(
                merged_df,
                price_col="price",
                sma_windows=(sma_fast, sma_slow),  # type: ignore ; impossible to be unbounded since it's slider
                rsi_period=int(rsi_period),  # type: ignore
                macd_fast=12,
                macd_slow=26,
                macd_signal=9,
            )
...
if (
    (not DEMO_MODE)
    and "merged_path" in st.session_state
    and os.path.exists(st.session_state["merged_path"])
):
    price_settings = {
        "dataset": "price",
        "coin": selected_coin,
    ...
    with sentiment:
        ...
        if "lead_lag_settings" in st.session_state:
            try:
                feats = load_or_build_lead_lag_features(
                    st.session_state["lead_lag_settings"],
                    st.session_state["merged_path"],
                )
            except Exception as e:
        ...
        # Sentiment timeline
        st.plotly_chart(
            plot_sentiment_timeline(df, selected_coin), use_container_width=True
        )

        # Sentiment vs price timeline (smoothed)
        st.plotly_chart(
            plot_sentiment_with_price(df, selected_coin), use_container_width=True
        )
    ...
    with finance:
        ...
        if backtest:
            bt, stats = run_backtest(
                df,
                cost_bps=cost_bps,  # type: ignore , impossible to be unbounded, is slider
                slippage_bps=slip_bps,  # type: ignore , impossible to be unbounded, is slider
                resample="5min",
            )
        ...
        if use_rsi:
            fig = plot_rsi(view, rsi_col=f"rsi_{int(rsi_period)}")  # type: ignore
            if fig:
                st.plotly_chart(fig, use_container_width=True)
