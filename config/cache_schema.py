...
DEFAULTS: Dict[
    str, Any
] = {  # Defaults - unused for now, could implement later but dont really see use
}
...
def _norm_value(key: str, v: Any) -> Any:
    ...
    if isinstance(v, str):
        return (
            v if key in ("start_date", "end_date") else v.lower()
        )  # if its not date makes it lower
    if isinstance(v, (list, tuple, set)):
        return (
            sorted([str(x).lower() for x in v]) or None
        )  # if its list, tuple or set, sorts them
...
def _canonicalize_settings(raw: Dict[str, Any]) -> Dict[str, Any]:
    if "dataset" not in raw:
        raise ValueError(
            "settings missing required key:dataset"
        )  # If no dataset = error
    ds = raw["dataset"]  # Takes in only keys required to be dataset
    if (
        ds not in HASH_KEYS_BY_DATASET
    ):  # Checks if used valid dataset name (like cant pass "prices" bcs dataset is "price")
        raise ValueError(f"Unknown dataset {ds}")

    out: Dict[str, Any] = {}  # Assigns empty dict
    for k in HASH_KEYS_BY_DATASET[ds]:
        v = _norm_value(
            k, raw.get(k, DEFAULTS.get(k))
        )  # normalize values inside the dictionary
