...
def merge_sentiment_and_price_df(
    price_df: pd.DataFrame, sentiment_df: pd.DataFrame
) -> pd.DataFrame:
    if sentiment_df.empty or price_df.empty:
        return pd.DataFrame(
            columns=["timestamp", "price", "text", "sentiment", "source"]
        )

    sentiment_df = sentiment_df.copy()
    price_df = price_df.copy()

    sentiment_df = normalize_timestamp_column(sentiment_df)

    price_df = normalize_timestamp_column(price_df)
    ...
    tol = (
        pd.Timedelta("30min")
        if span <= pd.Timedelta("2D")
        else (
            pd.Timedelta("12h") if span <= pd.Timedelta("14D") else pd.Timedelta("1D")
        )
    )
