...
def metrics(true_label: List[str], prediction: List[str]) -> Dict[str, Any]:
    ...
    report = classification_report(
        true_label, prediction, labels=list(CANONICAL), zero_division=0, digits=3
    )
    return {"accuracy": acc, "f1_macro": f1m, "confusion": cm, "report": report}


def _examples(
    y_true: Sequence[str], y_pred: Sequence[str], texts: Sequence[str], k: int = 4
...
def evaluate(
    ...
    for analyzer in ALL_ANALYZER_NAMES:
        ...
        m["time_sec"] = t1 - t0
        m["n_texts"] = len(texts)
        m["throughput_txt_per_s"] = (
            (len(texts) / (t1 - t0)) if (t1 - t0) > 0 else float("inf")
        )
